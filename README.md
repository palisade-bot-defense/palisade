...
Use `make load-test-plan` and then `make load-test-local BINARY=./bin/palisade`
for a bounded synthetic capacity diagnostic of the production-configured
session, proof and origin-check HTTP path. It emits only aggregate counters and
...
Before a production-configured start, generate all three credentials
independently and run `palisade doctor`. The command applies the same bounded
credential and separation checks as `serve`, performs no network or filesystem
I/O and never prints secret material. It validates only the base production
environment—not proxy/TLS configuration, signed artifacts, shadow-log paths or
enforcement readiness. See the [production environment preflight](docs/DOCTOR.md).
...
The required test pyramid, coverage and in-process latency gates are documented in [the testing strategy](docs/TESTING.md). The [synthetic benchmark protocol](docs/BENCHMARKS.md) records p50/p95/p99 and repeated allocation aggregates for exact clean commits with explicit non-production limitations. The [local release process](docs/RELEASING.md) now includes an exact, closed comparison of two unsigned candidates before checksum signing; that comparison does not by itself prove independent maintainers or hosts. All committed fixtures are synthetic; deployment logs and private analysis reports are excluded from tests and CI.

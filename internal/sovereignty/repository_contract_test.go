...
func TestDataMapIsClosedAndContainsNoRawAcceptedClass(t *testing.T) {
	...
	readRepositoryJSON(t, root, "manifests/data-map-v9.json", &manifest)
	if manifest.SchemaVersion != "palisade.data-map.v9" || manifest.Scope != "reference_product_data_flows" ||
...
func TestSovereigntyRepositorySchemasAreValidJSON(t *testing.T) {
	...
	for _, path := range []string{
		...
		"schemas/data-map-v9.schema.json",

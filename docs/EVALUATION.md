...
`palisade analyze-shadow-log` produces the closed aggregate `palisade.shadow-analysis.v2` readiness report from authenticated local records. It reports endpoint proportions with Wilson 95% intervals and compares each canary rollout/endpoint with the same endpoint's shadow-computed risky-action rate. A difference is marked comparable only when both groups are present in the retained window; its interval is the conservative combination of the two Wilson intervals, not a causal A/B estimate. Outcome shares remain event-level because `decision_id` is optional and a decision may produce several outcome events. They must not be called a false-positive rate or a per-session completion rate. Its default volume, coverage, confirmed-label and challenge-friction gates are operational collection checks, not a substitute for the metrics below. `operator_review_candidate` permits only review of a reversible endpoint canary; the report never enables enforcement.

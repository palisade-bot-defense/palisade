...
| [CrowdSec](https://doc.crowdsec.net/docs/intro/) | Open-source local behavior engine plus a collaborative threat-intelligence network and community blocklists. | PALISADE can consume a closed local reputation class but does not require sharing attack signals or joining a central identity/reputation graph. |
| [Anubis](https://github.com/TecharoHQ/anubis) | Focused, self-hosted challenge protection for sites facing aggressive AI crawlers. | PALISADE is broader and heavier: it separates automation from intent, measures delayed outcomes and governs rollout. Anubis remains a useful upstream signal rather than something to clone. |

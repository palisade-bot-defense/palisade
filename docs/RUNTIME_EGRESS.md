...
|---|---|---|---|
| Reference Go origin adapter | Operator-configured PALISADE base URL | Operator enables the adapter | Non-loopback plain HTTP is rejected; remote endpoints require HTTPS. The adapter sends closed requests, required backend credentials and opaque capabilities only. |

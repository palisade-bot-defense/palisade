...
if git grep -I -i -E 'PolyForm Shield|license pending|LICENSE-PENDING|"license"[[:space:]]*:[[:space:]]*"UNLICENSED"' -- ':!LICENSE' ':!sensor/LICENSE' ':!scripts/license-check.sh'; then
	echo "license-check: stale license wording found" >&2
	exit 1
fi

echo "license-check: AGPL core and Apache sensor boundary passed"
